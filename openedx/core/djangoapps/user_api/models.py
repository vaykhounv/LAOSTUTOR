...
class UserPreference(models.Model):
    ...
    def get_all_preferences(user):
        ...
        """
        return {pref.key: pref.value for pref in user.preferences.all()}  # lint-amnesty, pylint: disable=consider-using-dict-comprehension
...
class UserRetirementStatus(TimeStampedModel):
    ...
    def _validate_state_update(self, new_state):
        ...
        try:
            ...
            err = '{} does not exist or is an eariler state than current state {}'.format(
    ...
    def _validate_update_data(self, data):
        ...
        for required_key in required_keys:
            if required_key not in data:
                raise RetirementStateError('RetirementStatus: Required key {} missing from update'.format(
                    required_key
                ))
